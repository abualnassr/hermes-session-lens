...
This design document describes Hermes Session Lens `0.36.0`.
...
- **Periods and hidden sessions:** A period holds every session active in it (last active at or after its start, started before its end), because bot sessions run for days; Hermes records usage per session, not per day, so a straddling session counts whole and the daily bars place it on its last-active day. Sessions hidden from the Hermes sidebar count everywhere and are labelled in their detail header — hiding is a listing preference, not a refund.
